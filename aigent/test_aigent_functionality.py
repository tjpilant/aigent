...
def test_aigent_functionality():
    print("Starting AIGent functionality test")

    # Check if required modules are importable
    if find_spec("aigent") is None:
        print("Error: aigent module not found")
        return

    # Initialize AIGentSwarm
    try:
        swarm = AIGentSwarm()
        print("AIGentSwarm initialized successfully")
    except Exception as e:
        print(f"Error initializing AIGentSwarm: {str(e)}")
    ...
    if not os.path.exists(input_file):
        ...
        print(f"Created test document: {input_file}")
    ...
    print(f"Test parameters set up: input_files={input_files}, output_dir={output_dir}")
    ...
    try:
        ...
        print(f"Document processing completed. Results: {results}")
    except Exception as e:
    ...
    try:
        ...
        print(f"Training data generation completed. Results: {training_results}")
    except Exception as e:
        print(f"Error during training data generation: {str(e)}")
        print(f"Traceback: {traceback.format_exc()}")

    print("AIGent functionality test completed")
